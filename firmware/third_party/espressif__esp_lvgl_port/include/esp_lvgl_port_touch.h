...
typedef void (*lvgl_port_touch_points_cb_t)(
    const esp_lcd_touch_point_data_t *points, uint8_t count,
    uint32_t monotonic_ms, void *user_data);
...
/**
 * @brief Select timer polling while a dark screen must remain touch-wakeable.
 *
 * Interrupt/event mode is restored after wake so normal UI operation keeps
 * the lower input overhead.
 */
