...
extern void codex_display_monitor_flush_hook(const unsigned char *color_map,
        int x1, int y1, int x2, int y2, bool last_flush);
extern void codex_display_monitor_vsync_hook(unsigned int tick_count);
#endif
...
#if CONFIG_CODEX_DEVELOPMENT_HIL
    codex_display_monitor_vsync_hook((unsigned int)xTaskGetTickCountFromISR());
#endif
...
#if CONFIG_CODEX_DEVELOPMENT_HIL
    codex_display_monitor_flush_hook(color_map, offsetx1, offsety1, offsetx2,
                                     offsety2, lv_disp_flush_is_last(drv));
#endif
