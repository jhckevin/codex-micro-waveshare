...
esp_err_t lvgl_port_deinit(void)
{
    ...
    if (lvgl_port_ctx.running) {
        lvgl_port_ctx.deinit_waiter = xTaskGetCurrentTaskHandle();
        lvgl_port_ctx.running = false;
        if (lvgl_port_ctx.lvgl_events) {
            xEventGroupSetBits(lvgl_port_ctx.lvgl_events, 0xFF);
        }
        if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(2000)) == 0) {
            ESP_LOGE(TAG, "Timed out waiting for LVGL task shutdown");
            lvgl_port_ctx.deinit_waiter = NULL;
...
static void lvgl_port_task(void *arg)
{
    ...
    while (lvgl_port_ctx.running) {
        ...
        if (lv_display_get_default() && lvgl_port_lock(0)) {
            ...
            /* With the panel detached for Ultra standby there is no work to
             * poll. Sleep deeply until the port is given a display again. */
            task_delay_ms = 250;
        }
    ...
    TaskHandle_t deinit_waiter = lvgl_port_ctx.deinit_waiter;

    /* Deinit LVGL */
    lvgl_port_task_deinit();

    if (deinit_waiter) {
        xTaskNotifyGive(deinit_waiter);
    }
