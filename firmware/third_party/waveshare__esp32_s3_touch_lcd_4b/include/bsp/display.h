...
/**
 * @brief Release the hard-off latch during an explicit display wake.
 *
 * Ordinary brightness updates are ignored while the latch is active, so a
 * late renderer/settings update cannot relight a sleeping display.
 */
esp_err_t bsp_display_backlight_prepare_wake(void);

/**
 * @brief Enable or disable ST7701 panel output without tearing down RGB/LVGL.
 *
 * This is used for a real black standby while touch and host transports remain
 * alive. It is intentionally separate from the backlight PWM.
 */
