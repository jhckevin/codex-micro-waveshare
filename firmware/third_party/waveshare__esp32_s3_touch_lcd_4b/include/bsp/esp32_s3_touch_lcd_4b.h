...
/**
 * @brief Pause/resume the already configured I2S channels without rebuilding
 *        their DMA resources.
 */
...
/**
 * @brief Tear down touch, LVGL and the RGB panel for light sleep.
 *
 * The RGB panel owns an ESP_PM_NO_LIGHT_SLEEP lock for its lifetime, so merely
 * switching off the backlight is not sufficient. Call bsp_display_start() to
 * recreate the display after wake.
 */
esp_err_t bsp_display_stop(void);

/**
 * @brief Detach touch/RGB panel while retaining the initialized LVGL runtime.
 */
esp_err_t bsp_display_suspend(void);

/**
 * @brief Recreate touch/RGB panel on the retained LVGL runtime.
 */
