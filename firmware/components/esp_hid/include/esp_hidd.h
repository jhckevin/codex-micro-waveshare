...
/**
 * @brief Set BAS 1.1 battery level and power state reported by a BLE HID device.
 */
esp_err_t esp_hidd_dev_battery_status_set(
    esp_hidd_dev_t *dev, const esp_hid_battery_status_t *status);

/**
 * @brief Restore notification CCC state for an authenticated bonded BLE host.
 *
 * BLE CCC values are persistent per bonded client, but the Bluedroid HIDD
 * implementation keeps them only in RAM. Call this only after authentication
 * and after observing host HID output following a device reboot.
 *
 * @param dev : pointer to the BLE HID Device
 * @return: ESP_OK on success
 */
esp_err_t esp_hidd_dev_restore_bonded_cccs(esp_hidd_dev_t *dev);
