...
static const uint16_t s_bat_level_status_uuid = 0x2BED;
...
enum {
    ...
    BAS_IDX_BATT_STATUS_CHAR,
    BAS_IDX_BATT_STATUS_VAL,
    BAS_IDX_BATT_STATUS_CCC,
...
struct esp_ble_hidd_dev_s {
    ...
    hidd_le_ccc_value_t         bat_status_ccc;
    uint8_t                     bat_level;  // 0 - 100 - battery percentage
    uint8_t                     bat_status[4];
    uint8_t                     bat_status_len;
    ...
    uint16_t                    bat_status_handle;
    uint16_t                    bat_status_ccc_handle;
...
static esp_err_t esp_ble_hidd_dev_battery_status_set(
    void *devp, const esp_hid_battery_status_t *status);
...
static esp_err_t create_bat_db(esp_ble_hidd_dev_t *dev)
{
    ...
    add_db_record(_last_db, BAS_IDX_BATT_STATUS_CHAR, (uint8_t *)&s_character_declaration_uuid, ESP_GATT_PERM_READ, 1, 1, (uint8_t *)&s_char_prop_read_notify);
    add_db_record(_last_db, BAS_IDX_BATT_STATUS_VAL, (uint8_t *)&s_bat_level_status_uuid, ESP_GATT_PERM_READ, sizeof(dev->bat_status), dev->bat_status_len, dev->bat_status);
    add_db_record(_last_db, BAS_IDX_BATT_STATUS_CCC, (uint8_t *)&s_character_client_config_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, 2, 0, NULL);
...
static void bat_event_handler(esp_ble_hidd_dev_t *dev, esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:
        ...
        dev->bat_status_handle = param->add_attr_tab.handles[BAS_IDX_BATT_STATUS_VAL];
        dev->bat_status_ccc_handle = param->add_attr_tab.handles[BAS_IDX_BATT_STATUS_CCC];
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        } else if (param->write.handle == dev->bat_status_ccc_handle) {
            dev->bat_status_ccc.value = param->write.value[0];
            ESP_LOGV(TAG, "Battery Status CCC: Notify: %s, Indicate: %s",
                     dev->bat_status_ccc.notify_enable ? "On" : "Off",
                     dev->bat_status_ccc.indicate_enable ? "On" : "Off");
        }
        break;
    }
...
static esp_err_t esp_ble_hidd_dev_battery_set(void *devp, uint8_t level)
{
    ...
    return esp_ble_hidd_dev_battery_status_set(devp, &status);
}
    ...
    void *devp, const esp_hid_battery_status_t *status)
{
    esp_err_t ret;
    esp_ble_hidd_dev_t *dev = (esp_ble_hidd_dev_t *)devp;
    if (!dev || s_dev != dev || !status) {
        return ESP_ERR_INVALID_ARG;
    }
    dev->bat_level = status->level > 100 ? 100 : status->level;
    const esp_hid_battery_level_status_value_t value =
        esp_hid_make_battery_level_status(status->present, dev->bat_level,
                                          status->charging,
                                          status->external_power);
    memcpy(dev->bat_status, value.bytes, sizeof(dev->bat_status));
    dev->bat_status_len = value.length;

    if (dev->bat_level_handle != 0) {
        ret = esp_ble_gatts_set_attr_value(dev->bat_level_handle, 1,
                                           &dev->bat_level);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "set Battery Level value failed: %d", ret);
            return ret;
        }
    }
    if (dev->bat_status_handle != 0) {
        ret = esp_ble_gatts_set_attr_value(dev->bat_status_handle,
                                           dev->bat_status_len,
                                           dev->bat_status);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "set Battery Level Status value failed: %d", ret);
            return ret;
        }
    }
    ...
    if (dev->bat_status_ccc.notify_enable) {
        ret = esp_ble_gatts_send_indicate(
            dev->bat_svc.gatt_if, dev->conn_id, dev->bat_status_handle,
            dev->bat_status_len, dev->bat_status, false);
        if (ret) {
            ESP_LOGE(TAG, "Battery Level Status notify failed: %d", ret);
            return ESP_FAIL;
        }
    }
...
esp_err_t esp_ble_hidd_dev_restore_bonded_cccs(void *devp)
{
    esp_ble_hidd_dev_t *dev = (esp_ble_hidd_dev_t *)devp;
    ...
    size_t restored = 0;
    for (uint8_t map_index = 0; map_index < dev->devices_len; ++map_index) {
        hidd_dev_map_t *map = &dev->devices[map_index];
        for (uint8_t report_index = 0; report_index < map->reports_len;
             ++report_index) {
            hidd_le_report_item_t *report = &map->reports[report_index];
            if (report->report_type == ESP_HID_REPORT_TYPE_INPUT) {
                report->ccc.value = 0;
                report->ccc.notify_enable = 1;
                ++restored;
            }
        }
    }
    dev->bat_ccc.value = 0;
    dev->bat_ccc.notify_enable = 1;
    dev->bat_status_ccc.value = 0;
    dev->bat_status_ccc.notify_enable = 1;
    ESP_LOGI(TAG, "Restored bonded CCC state for %u input reports",
             (unsigned int)restored);
    return restored == 0 ? ESP_ERR_NOT_FOUND : ESP_OK;
}
...
esp_err_t esp_ble_hidd_dev_init(esp_hidd_dev_t *dev_p, const esp_hid_device_config_t *config, esp_event_handler_t callback)
{
    ...
    s_dev->bat_status_len = 3;
    s_dev->bat_status_ccc.value = 0;
    ...
    dev_p->battery_status_set = esp_ble_hidd_dev_battery_status_set;
