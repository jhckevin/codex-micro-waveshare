    ...
    esp_hidd_dev_t *dev, const esp_hid_battery_status_t *status)
{
    ...
    if (dev->battery_status_set != NULL) {
        return dev->battery_status_set(dev->dev, status);
    }
    return dev->battery_set(dev->dev, status->level);
}
...
esp_err_t esp_hidd_dev_restore_bonded_cccs(esp_hidd_dev_t *dev)
{
    ...
    if (dev->transport == ESP_HID_TRANSPORT_BLE) {
        return esp_ble_hidd_dev_restore_bonded_cccs(dev->dev);
    }
#endif
